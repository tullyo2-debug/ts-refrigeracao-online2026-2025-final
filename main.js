...
if (!url || !key) {
  document.querySelector('#app').innerHTML = `<main class="setup"><h1>TS Refrigeração</h1><p>Configure <code>.env</code> com VITE_SUPABASE_URL e VITE_SUPABASE_PUBLISHABLE_KEY.</p></main>`
...
function clientForm(x={}) {
  ...
  <div class="grid2"><label>Nome*<input id="nome" value="${esc(x.nome)}"></label><label>Tipo<select id="tipo"><option ${x.tipo_pessoa==='PJ'?'selected':''}>PF</option><option ${x.tipo_pessoa==='PJ'?'selected':''}>PJ</option></select></label>
...
async function agenda(c) {
  const {data,error}=await supabase.from('agendamentos').select('*,clientes(nome),usuarios(nome),equipamentos(tipo,marca)').order('data_agendamento').order('hora_inicio')
  if(error)throw error
...
async function ordens(c) {
  const {data,error}=await supabase.from('ordens_servico').select('*,clientes(nome),usuarios(nome)').order('criado_em',{ascending:false}).limit(100)
  if(error)throw error
...
async function osForm() {
  ...
  document.querySelector('#saveos')?.addEventListener('click',async()=>{
    ...
    const payload={cliente_id:cliente,tecnico_id:(document.querySelector('#tech')).value||null,status:'aberta',prioridade:(document.querySelector('#priority')).value,problema_relatado:(document.querySelector('#problem')).value,diagnostico:(document.querySelector('#diag')).value,servico_executado:(document.querySelector('#exec')).value,observacoes:(document.querySelector('#obs')).value,valor_servicos:valor,valor_pecas:0,desconto:0,valor_total:valor,criado_por:user.id}
